...
from core.models import Task, UploadResult, UploadStatus
from core.providers.base import ProviderUpload, UploadProvider


@dataclass(frozen=True, slots=True)
class UploadSummary:
    """Final outcome of all requested provider uploads."""
...
class UploadManager:
    """Upload one downloaded file to multiple providers.

    Providers run concurrently up to ``max_concurrency``. The local file is
    removed only when every requested provider succeeds, preventing a failed
    upload from forcing another download.
    """

    def __init__(
        self,
        providers: dict[str, UploadProvider],
        *,
    ...
    async def upload_task(self, task: Task) -> UploadSummary:
        """Upload a task's local file to every selected provider."""
        ...
        selected = list(dict.fromkeys(task.selected_providers))
        if not selected:
            return UploadSummary([], True, False)

        missing = [provider_id for provider_id in selected if provider_id not in self.providers]
        if missing:
            raise ValueError(f"Unknown providers: {', '.join(missing)}")

        semaphore = asyncio.Semaphore(self.max_concurrency)
        results = await asyncio.gather(
            *(self._upload_one(task, provider_id, semaphore) for provider_id in selected)
        )
        task.uploads = results

        all_succeeded = all(result.status is UploadStatus.COMPLETED for result in results)
        cleaned_up = False
        if all_succeeded:
